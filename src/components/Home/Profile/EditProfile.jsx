...
const EditProfile = ({ editModal, setEditModal, getUserData }) => {
  ...
  const saveForm = async () => {
    ...
    try {
      ...
      if (form.userImg && typeof form.userImg === 'object') {
        ...
        const b64 = await toBase64(form.userImg);
        const upRes = await fetch(`${base}/api/upload/imgbb`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ file: b64, name: form.username })
        });
      ...
      // Update user profile
      const res = await fetch(`${base}/api/users/${currentUser.id || currentUser.uid}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          username: form.username,
          bio: form.bio,
          userImg: imageUrl || getUserData?.userImg || '/profile.jpg'
        })
      });
    ...
    <Modal modal={editModal} setModal={setEditModal}>
      <div
        ...
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-xl heading-ui">Profile information</h2>
          ...
          <div className="flex gap-[2rem]">
            <div className="w-[5rem]">
              <img
                ...
                src={
                  imgUrl ? imgUrl : form.userImg ? form.userImg : getUserData?.userImg || getUserData?.avatar || "/profile.jpg"
                }
                alt="profile-img"
              />
              <input
