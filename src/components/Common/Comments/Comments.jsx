...
const Comments = ({ postId }) => {
  ...
  const { data, loading } = useSingleFetch("comments", postId, "");
    ...
    try {
      ...
      const res = await fetch("http://localhost:3000/api/comments", {
        ...
        body: JSON.stringify({
          ...
          userImg: getUserData?.userImg || getUserData?.avatar || "/profile.jpg",
        }),
      });
      if (!res.ok) throw new Error("Failed to add comment");
      toast.success("Comment has been added");
      setComment("");
      // Refresh comments data by triggering a re-fetch
      setTimeout(() => {
    ...
    <Modal setModal={setShowComment} modal={showComment}>
      ...
      `}>
        ...
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-bold heading-ui">Responses({data.length})</h3>
          ...
          <div className="shadows p-3 my-5 overflow-hidden">
            <div className="flex items-center gap-2 mb-5">
              <img
                ...
                alt="user-img"
              />
              <h3 className="capitalize text-sm ui-text">{getUserData?.username}</h3>
            </div>
            <textarea
              value={comment}
